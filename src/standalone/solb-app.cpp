...
main(int argc, char** argv)
{
    ...
    {
        if ((o_fp = fopen(arguments.output_file, "rt")) == NULL)
        {
     ...
     */
    mag_field_2d_t *res_chunk;
    size_t rem = nprobe % CHUNK_SIZE;
    size_t full_chunks = nprobe / CHUNK_SIZE;
    fprintf(o_fp, "%16s%16s%16s%16s", "Coord_R", "Coord_Z", "Br", "Bz");
    if (full_chunks == 0)
    {
#pragma omp parallel for
        ...
        for (size_t i = 0; i < nprobe; ++i)
        {
            fprintf(o_fp, "%16lf%16lf%16lf%16lf", probes[i].r, probes[i].z, 
        }
    }
    else
    {
